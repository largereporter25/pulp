 ...
 * Mirrors Final Draft / Fountain conventions for element switching,
...
export const ELEMENT_SHORT: Record<ScreenplayElementType, string> = {
  scene: "Scene",
  action: "Action",
  character: "Character",
  dialogue: "Dialogue",
...
// Single-key shortcut shown in UI. We bind Alt/Ctrl+number AND a command menu.
export const ELEMENT_NUMBER: Record<ScreenplayElementType, number> = {
  scene: 1,
  action: 2,
  character: 3,
  parenthetical: 4,
...
// Common transitions for the transition quick-menu.
...
// Scene-heading prefixes for the scene quick-menu / autocomplete.
...
export function autoDetect(text: string, current: ScreenplayElementType): ScreenplayElementType | null {
  ...
  if (current === "action" || current === "transition") {
    if (/(TO:|FADE OUT\.?|FADE IN:|CUT TO:|DISSOLVE TO:|SMASH CUT TO:)$/.test(t)) return "transition";
  }
  return null;
}
